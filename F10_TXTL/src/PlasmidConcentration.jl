...
flux_model_dictionary["PROTEIN_export_F10"].flux_obj_coeff = 1.0;

# set the bound -
#data_dictionary["M_o2_c_exchange_reverse"] = 100
data_dictionary["translation_F10_switch_bound"] = false

# Setup steady-state array -
plasmind_concentration_array = collect(linspace(0,2,number_of_samples));
production_time = 10;
F10_array = zeros(length(plasmind_concentration_array))
...
for (index,plasmid_concentration) in enumerate(plasmind_concentration_array)
  ...
  data_dictionary["F10_gene_copies"] = gene_copy_number;

  # Run the simulation -
  (OV,FA,DA,UA,EF) = Solve(data_dictionary);

  # calculate the protein concentration -
  F10_concentration = (1e3)*OV*production_time;

  # capture -
  F10_array[index] = F10_concentration;
...
writedlm("../Conc.txt",(plasmind_concentration_array,F10_array))

# plot -
clf()

plot(plasmind_concentration_array,F10_array,linewidth="2.0")
xlabel("Plasmid concentration [nM]",fontsize="14")
ylabel(L"Steady-state F10 concentration [$\mu$M]",fontsize="14")
