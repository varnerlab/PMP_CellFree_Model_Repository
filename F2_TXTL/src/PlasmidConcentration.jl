...
flux_model_dictionary["PROTEIN_export_F2"].flux_obj_coeff = 1.0;

# set the bound -
#data_dictionary["M_o2_c_exchange_reverse"] = 100
data_dictionary["translation_F2_switch_bound"] = false

# Setup steady-state array -
plasmind_concentration_array = collect(linspace(0,2,number_of_samples));
production_time = 10;
F2_array = zeros(length(plasmind_concentration_array))
...
for (index,plasmid_concentration) in enumerate(plasmind_concentration_array)
  ...
  data_dictionary["F2_gene_copies"] = gene_copy_number;

  # Run the simulation -
  (OV,FA,DA,UA,EF) = Solve(data_dictionary);

  # calculate the protein concentration -
  F2_concentration = (1e3)*OV*production_time;

  # capture -
  F2_array[index] = F2_concentration;
...
writedlm("../Conc.txt",(plasmind_concentration_array,F2_array))

# plot -
clf()

plot(plasmind_concentration_array,F2_array,linewidth="2.0")
xlabel("Plasmid concentration [nM]",fontsize="14")
ylabel(L"Steady-state F2 concentration [$\mu$M]",fontsize="14")
