...
flux_model_dictionary["PROTEIN_export_FGF21"].flux_obj_coeff = 1.0;
...
production_time = 10;
FGF21_array = zeros(length(plasmind_concentration_array))
...
for (index,plasmid_concentration) in enumerate(plasmind_concentration_array)
  ...
  data_dictionary["FGF21_gene_copies"] = gene_copy_number;

  # Run the simulation -
  (OV,FA,DA,UA,EF) = Solve(data_dictionary);

  # calculate the protein concentration -
  FGF21_concentration = (1e3)*OV*production_time;

  # capture -
  FGF21_array[index] = FGF21_concentration;
...
writedlm("../Conc.txt",(plasmind_concentration_array,FGF21_array))

# plot -
clf()

plot(plasmind_concentration_array,FGF21_array,linewidth="2.0")
xlabel("Plasmid concentration [nM]",fontsize="14")
ylabel(L"Steady-state FGF21 concentration [$\mu$M]",fontsize="14")
